...
router.put('/weight', function(req, response) {
    if (!isNaN(parseFloat(req.body.weight)) && isFinite(req.body.weight)) {
        fs.appendFile('/opt/weightData.txt', req.body.weight+','+(new Date()).getTime()+'\n', function (err) {
            if (err) {
                console.log(err);
                response.status(400);
                response.send({});
...
router.put('/message', function(req, response) {
    var body = req.body;
    body.time = (new Date()).getTime();
    fs.appendFile('/opt/messages.txt', JSON.stringify(body) + '\n', function (err) {
        if (err) {
            console.log(err);
            response.status(400);
            response.send({});
...
router.get('/weight', function(req, response) {
    var resData = [];
    fs.readFile('/opt/weightData.txt', 'utf8', function(err, data) {
        data.split('\n').forEach(function(line) {
            var parts = line.split(',');
            if(parts.length === 2) {
                resData.push({weight: parts[0], date: parts[1]})
            }
        });
        response.status(200);
